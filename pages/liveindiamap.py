...
layout = html.Div([html.Div([html.H2("India Live Map")],
                       ...
                       html.Div([html.Span("Different cases to display : ", className="six columns",
                                 ...
                                 dcc.Dropdown(id="state-selected", value='Confirmed',
...
def update_figure(selected):
    dff = df_india.groupby(['State', 'Confirmed','Recovered','Deaths']).max().reset_index()
    def title(text):
        if text == "Confirmed":
            ...
            return "Recovered"
    trace = go.Choroplethmapbox(geojson=states, featureidkey= "properties.State",locations=dff.State, z=dff[selected],
                                    colorscale=colormap, marker_line_width=1)
    return {"data": [trace],
            "layout": go.Layout(mapbox_style='mapbox://styles/cocktailsguy/ck8edouiv02lv1io9quvrr36i', mapbox_accesstoken=token,
                  mapbox_zoom=3.1, mapbox_center = {"lat": 21, "lon": 78},margin={"r":0,"t":0,"l":0,"b":0})}

...
df_states.columns = ['United_States', 'Confirmed','New Confirmed','Deaths', 'New Deaths','Serious & Critical','Recovered']


#df_states = df.loc[5:, :]
#df_states.columns = ['United_States', 'Confirmed', 'Deaths','Serious','Critical','Recovered']
df_states = df_states[:-3]

df_states['iso_alpha'] = df_states['United_States'].apply(lambda x: coco.convert(names=x, src='regex', to='ISO2', not_found = None))
#df_states['iso_alpha'] = df_states['United States'].apply(lambda x: pc.countries.search_fuzzy(x))

labels = df_states['United_States'].unique()
#labels_dict = dict(zip(us_state_abbrev,us_state_abbrev.items()))
...
layout = html.Div([html.Div([html.H2("USA Live Corona Cases")],
...
colormap = [[0.0, color1], [0.05, color2], [0.45, color3],
...
def update_figure(selected):
    dff = df_states.groupby(['iso_alpha','United_States','Confirmed', 'Deaths','Serious & Critical','Recovered']).max().reset_index()
    def title(text):
        if text == "Confirmed":
            ...
            return "Recovered"
    trace = go.Choropleth(locations=dff['iso_alpha'],z=dff[selected],text=dff['United_States'],autocolorscale=False,
                          locationmode='USA-states',colorscale=colormap,marker={'line': {'color': 'rgb(180,180,180)','width': 1.5}},
                          colorbar={"thickness": 10,"len": 0.3,"x": 0.9,"y": 0.7,
                                    'title': {"text": title(selected), "side": "bottom"},
                                    'titleside' : 'top',
                                    'ticks' : 'outside'})
    return {"data": [trace],
            "layout": go.Layout(title=title(selected),height=800,geo={'scope':'usa','showframe': False,
